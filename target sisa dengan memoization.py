...
class Stats:
    ...
    def add(self, other: "Stats"):
        ...
        self.modulo_prunes += other.modulo_prunes
...
def lower_bound_sum(values, start, remainder):
    n = len(values)
    idx = bisect.bisect_right(values, remainder, lo=start, hi=n) - 1
    if idx < start:
        return None, 0
    max_val = values[idx]
    k = (remainder + max_val - 1) // max_val
    available = idx - start + 1
    if k > available:
        return None, 0
    return k, available

def _solve_subtree_worker(args):
    (
        values,
        target,
        start_idx,
    ...
    def dfs(start, remainder, chosen):
        ...
        k, avail = lower_bound_sum(values, start, remainder)
        stats.bound_checks += 1
        if k is None:
...
def target_remainder_advanced(values, target, collect_solutions=True, num_workers=None):
    ...
    if num_workers <= 1 or len(tasks) <= 1 or n <= 12:
        for t in tasks:
            sub_stats, sub_sols = _solve_subtree_worker(t)
            stats.add(sub_stats)
            if collect_solutions:
                solutions.extend(sub_sols)
    else:
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            results = executor.map(_solve_subtree_worker, tasks)
...
class SubsetSumApp:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Subset Sum — Target-Sisa Memoization (Method 3)")
    ...
    def _create_widgets(self):
        ...
        title_lbl = tk.Label(
            title_box,
            text="SUBSET SUM SOLVER — TARGET-SISA MEMOIZATION",
        ...
        sub_lbl = tk.Label(
            title_box,
            text="Metode 3: Target-Sisa Memoization (Dead States) • Pemrosesan Multi-Core",
            font=("Segoe UI", 9),
            bg="#0f172a",
            fg="#94a3b8"
        )
    ...
    def _show_initial_help(self):
        ...
        self.stat_text.insert(
            tk.END,
            "SELAMAT DATANG DI SUBSET SUM SOLVER — TARGET-SISA MEMOIZATION (MULTI-CORE & ASYNC)\n"
            "========================================================================\n"
            "Klik tombol '▶ Jalankan Algoritma' untuk memulai pencarian subset sum.\n\n"
            "Karakteristik Metode 3 (Target-Sisa Memoization):\n"
            "  • Pencarian Target-Sisa dengan Dead-States Memoization Cache.\n"
    ...
    def _start_calculation(self):
        ...
        self.summary_lbl.config(
            text="Memproses pohon pencarian Target-Sisa Memoization. Waktu berjalan: 0.0s"
        )
    ...
    def _run_computation_thread(self, values, target):
        try:
            stats, solutions = target_remainder_advanced(
                values,
                target,
    ...
    def _show_statistics(self):
        ...
        lines.append("=" * 76)
        lines.append("HASIL KOMPUTASI SUBSET SUM — TARGET-SISA MEMOIZATION (MULTI-CORE)")
        lines.append("=" * 76)
        ...
        lines.append(f"  • Dead-States Memo Hits     : {s.memo_hits:>16,}")
        lines.append(f"  • Total Work Units          : {s.work_units:>16,}")
        lines.append("")

        lines.append("-" * 76)
        lines.append("RINCIAN PEMANGKASAN CABANG")
        lines.append("-" * 76)
        ...
        lines.append("  [✓] Dead-States Memoization")
    ...
    def _open_graph_window(self):
        ...
        tk.Label(
            hdr,
            text="Visualisasi Aktivitas & Beban Komputasi Target-Sisa Memoization",
            font=("Segoe UI", 11, "bold"),
            bg="#6d28d9",
            fg="#ffffff"
        ).pack(anchor="w")
    ...
    def _update_graph_window(self):
        ...
        ax.set_title(f"Distribusi Operasi Komputasi Target-Sisa Memoization ({self.total_cores} Core CPU)", fontsize=11, fontweight="bold", pad=12)
    ...
    def _save_txt(self):
        ...
        lines.append("=" * 80)
        lines.append("SUBSET SUM — TARGET-SISA MEMOIZATION (MULTI-CORE)")
        lines.append("=" * 80)
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile=f"target_sisa_memoization_T{self.target}_n{len(self.values)}.txt"
        )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Subset Sum — Target-Sisa Memoization (Method 3)")
    ...
    if args.cli:
        ...
        stats, sols = target_remainder_advanced(vals, args.target, collect_solutions=True)
        print("=" * 76)
        print("  TARGET-SISA MEMOIZATION (METHOD 3) — CLI")
        print("=" * 76)
