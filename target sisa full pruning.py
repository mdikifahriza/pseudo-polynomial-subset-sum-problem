...
def _solve_full_pruning_worker(args):
    (
        values,
        target,
        start_idx,
    ...
    def dfs(start, remainder, chosen):
        ...
        if suffix_sum[start] == remainder:
            stats.solutions += 1
            if collect_solutions:
                solutions.append(tuple(chosen + values[start:n]))
            return True
        ...
        stats.comparisons += 1
        if values[start] > remainder:
            stats.lower_prunes += 1
            stats.pruned += (n - start)
            return False

        max_cand_idx = bisect.bisect_right(values, remainder, lo=start, hi=n) - 1
        if max_cand_idx < start:
            stats.lower_prunes += 1
            stats.pruned += (n - start)
            return False

        available_count = max_cand_idx - start + 1
        ...
        cand_sum = suffix_sum[start] - suffix_sum[max_cand_idx + 1]
...
def target_remainder_full_pruning(values, target, collect_solutions=True, num_workers=None):
    ...
    if num_workers <= 1 or len(tasks) <= 1 or n <= 12:
        for t in tasks:
            sub_stats, sub_sols = _solve_full_pruning_worker(t)
            stats.add(sub_stats)
            if collect_solutions:
                solutions.extend(sub_sols)
    else:
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            results = executor.map(_solve_full_pruning_worker, tasks)
...
class TargetSisaFullPruningApp:

    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Subset Sum — Target-Sisa Full Pruning (Method 4)")
    ...
    def _create_widgets(self):
        ...
        title_lbl = tk.Label(
            title_box,
            text="SUBSET SUM SOLVER — FULL PRUNING (TANPA MEMO)",
        ...
        sub_lbl = tk.Label(
            title_box,
            text="Metode 4: Full Sound Pruning (Sum, GCD, Lower, Cardinality) • Pemrosesan Multi-Core",
            font=("Segoe UI", 9),
            bg="#0f172a",
            fg="#94a3b8"
        )
    ...
    def _show_initial_help(self):
        ...
        self.stat_text.insert(
            tk.END,
            "SELAMAT DATANG DI METODE 4: TARGET-SISA FULL PRUNING (MULTI-CORE & ASYNC)\n"
            "========================================================================\n"
            "Klik tombol '▶ Jalankan Algoritma' untuk memulai pencarian subset sum.\n\n"
            "Karakteristik Metode 4 (Full Pruning Tanpa Memo):\n"
            "  • 5 Aturan Pemangkasan Matematis Sound (Suffix Sum, GCD, Lower, Cardinality, Greedy).\n"
            "  • 100% Bebas Memoization (Pencarian pohon murni berorientasi target-sisa).\n"
    ...
    def _start_calculation(self):
        ...
        self.summary_lbl.config(
            text="Memproses pohon pencarian Full Pruning secara paralel. Waktu berjalan: 0.0s"
        )
    ...
    def _update_live_timer(self):
        ...
        self.summary_lbl.config(
            text=f"Mengevaluasi cabang Full Pruning di seluruh core CPU. Waktu berjalan: {elapsed:.1f} detik"
        )
        self.root.after(100, self._update_live_timer)

    def _run_computation_thread(self, values, target):
        try:
            stats, solutions = target_remainder_full_pruning(
                values,
                target,
    ...
    def _show_statistics(self):
        ...
        lines.append("=" * 76)
        lines.append("HASIL KOMPUTASI SUBSET SUM — TARGET-SISA FULL PRUNING (MULTI-CORE)")
        lines.append("=" * 76)
        ...
        lines.append("-" * 76)
        lines.append("RINCIAN PEMANGKASAN CABANG (FULL PRUNING)")
        lines.append("-" * 76)
        ...
        lines.append("  [✓] Suffix GCD Sound Pruning")
        lines.append("  [✓] Candidate Lower Bound Pruning")
        lines.append("  [✓] Cardinality Bound Pruning")
        lines.append("  [✓] Greedy Candidate Sum Upper Bound")
    ...
    def _open_graph_window(self):
        ...
        tk.Label(
            hdr,
            text="Visualisasi Aktivitas & Beban Komputasi Full Pruning",
            font=("Segoe UI", 11, "bold"),
            bg="#6d28d9",
            fg="#ffffff"
        ).pack(anchor="w")
    ...
    def _update_graph_window(self):
        ...
        ax.set_title(f"Distribusi Operasi Komputasi Full Pruning ({self.total_cores} Core CPU)", fontsize=11, fontweight="bold", pad=12)
    ...
    def _save_txt(self):
        ...
        lines.append("=" * 80)
        lines.append("SUBSET SUM — TARGET-SISA FULL PRUNING (MULTI-CORE)")
        lines.append("=" * 80)
        ...
        lines.append(f"  - Cardinality   : {s.cardinality_prunes:,}")
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile=f"target_sisa_full_pruning_T{self.target}_n{len(self.values)}.txt"
        )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Subset Sum — Target-Sisa Full Pruning (Method 4)")
    ...
    if args.cli:
        ...
        stats, sols = target_remainder_full_pruning(vals, args.target, collect_solutions=True)
        print("=" * 76)
        print("  TARGET-SISA FULL PRUNING (METHOD 4) — CLI")
        print("=" * 76)
