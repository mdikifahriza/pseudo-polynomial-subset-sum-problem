...
def _solve_bruteforce_worker(args):
    (
        values,
        target,
        start_idx,
    ...
    def dfs(i, current_sum, chosen):
        ...
        dfs(i + 1, current_sum + values[i], chosen)
        chosen.pop()

        dfs(i + 1, current_sum, chosen)

    dfs(start_idx, initial_sum, list(initial_chosen))
    return stats, solutions

def solve_brute_force(values, target, collect_solutions=True, num_workers=None):
    ...
    if num_workers <= 1 or len(tasks) <= 1 or n <= 10:
        for t in tasks:
            sub_stats, sub_sols = _solve_bruteforce_worker(t)
            stats.add(sub_stats)
            if collect_solutions:
                solutions.extend(sub_sols)
    else:
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            results = executor.map(_solve_bruteforce_worker, tasks)
...
class BruteForceApp:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Subset Sum — Brute Force (Method 1)")
    ...
    def _create_widgets(self):
        ...
        title_lbl = tk.Label(
            title_box,
            text="SUBSET SUM SOLVER — BRUTE FORCE",
        ...
        sub_lbl = tk.Label(
            title_box,
            text="Metode 1: Brute Force Full Enumeration • Pemrosesan Paralel Multi-Core",
            font=("Segoe UI", 9),
            bg="#0f172a",
            fg="#94a3b8"
        )
    ...
    def _show_initial_help(self):
        ...
        self.stat_text.insert(
            tk.END,
            "SELAMAT DATANG DI SUBSET SUM SOLVER — BRUTE FORCE (MULTI-CORE & ASYNC)\n"
            "========================================================================\n"
            "Klik tombol '▶ Jalankan Algoritma' untuk memulai pencarian subset sum.\n\n"
            "Karakteristik Metode 1 (Brute Force):\n"
            "  • Eksplorasi seluruh kombinasi 2^n secara biner (Include/Exclude).\n"
    ...
    def _start_calculation(self):
        ...
        self.summary_lbl.config(
            text="Memproses pohon pencarian Brute Force secara paralel. Waktu berjalan: 0.0s"
        )
    ...
    def _update_live_timer(self):
        ...
        self.summary_lbl.config(
            text=f"Mengevaluasi cabang biner 2^n di seluruh core CPU. Waktu berjalan: {elapsed:.1f} detik"
        )
        self.root.after(100, self._update_live_timer)

    def _run_computation_thread(self, values, target):
        try:
            stats, solutions = solve_brute_force(
                values,
                target,
    ...
    def _show_statistics(self):
        ...
        lines.append("=" * 76)
        lines.append("HASIL KOMPUTASI SUBSET SUM — BRUTE FORCE (MULTI-CORE)")
        lines.append("=" * 76)
        ...
        lines.append("=" * 76)
        lines.append("KARAKTERISTIK METODE")
        lines.append("=" * 76)
        lines.append("  [✓] Brute Force Full Enumeration Multi-Core")
        lines.append("  [✗] Tanpa Pruning Cerdas")
    ...
    def _open_graph_window(self):
        ...
        tk.Label(
            hdr,
            text="Visualisasi Aktivitas & Beban Komputasi Brute Force",
            font=("Segoe UI", 11, "bold"),
            bg="#6d28d9",
            fg="#ffffff"
        ).pack(anchor="w")
    ...
    def _update_graph_window(self):
        ...
        ax.set_title(f"Distribusi Operasi Komputasi Brute Force ({self.total_cores} Core CPU)", fontsize=11, fontweight="bold", pad=12)
    ...
    def _save_txt(self):
        ...
        lines.append("=" * 80)
        lines.append("SUBSET SUM — BRUTE FORCE (MULTI-CORE)")
        lines.append("=" * 80)
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile=f"bruteforce_T{self.target}_n{len(self.values)}.txt"
        )
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Subset Sum — Brute Force (Method 1)")
    ...
    if args.cli:
        ...
        stats, sols = solve_brute_force(vals, args.target, collect_solutions=True)
        print("=" * 76)
        print("  BRUTE FORCE (METHOD 1) — CLI")
        print("=" * 76)
